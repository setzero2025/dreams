 ...
 * 使用方法: node scripts/fix-theme.js
 */
...
const replacements = [
  ...
  // 组件开头添加 useTheme
  {
    from: /export\s+const\s+(\w+):\s*React\.FC[^>]+>\s*=\s*\(\s*\{[^}]+\}\s*\)\s*=>\s*\{/g,
    to: (match, name) => `export const ${name}: React.FC<any> = ({ navigation, route }) => {\n  const { colors, isDark } = useTheme();`
  },
...
// 执行修复
console.log('🎨 开始修复主题兼容性...\n');

filesToFix.forEach(fixFile);

console.log('\n✨ 主题修复完成！');
console.log('提示: 请检查文件确保替换正确，特别是动态样式部分。');
